...
<div class="container pt-4" style="width: 73%">
	...
	<c:choose>
		<c:when test="${empty search}">
			<h2 class="pb-3">3355</h2>
		</c:when>
		<c:otherwise>
			<h2 class="pb-3">"${search}" 3355</h2>
		</c:otherwise>
	</c:choose>
  <a href="/board/writeForm"><button class="btn btn-dark btn-sm mt-3">글쓰기</button></a>
  </div>
	  	...
	  	<c:choose>
	  		...
	  		<c:otherwise>
	  			<c:choose>
		  	  	...
		  	  	<c:otherwise>
		  	  		<li class="page-item"><a class="page-link" href="?page=${boards.number-1}&search=${param.search}">Prev</a></li>
		  	  	</c:otherwise>
		  	  </c:choose>
		  	  ...
		  	  <c:forEach begin="${startBlockPage}" end="${endBlockPage}" var="i">
		  	  	<c:choose>
		  	  		...
		  	  		<c:otherwise>
		  	  			<li class="page-item"><a class="page-link" href="?page=${i-1}&search=${param.search}">${i}</a></li>
		  	  		</c:otherwise>
		  	  	</c:choose>
		  	  </c:forEach>
		  	  ...
		  	  <c:choose>
		  	  	...
		  	  	<c:otherwise>
		  	  		<li class="page-item"><a class="page-link" href="?page=${boards.number+1}&search=${param.search}">Next</a></li>
		  	  	</c:otherwise>
		  	  </c:choose>
	  		</c:otherwise>
	  	</c:choose>
